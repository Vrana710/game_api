...
from models import db, User, Character, Role, Strength, House
from blueprints.utils import fetch_character_data
...
def handle_file_upload(upload_request):
    if 'profile_picture' in upload_request.files:
        ...
        if file and allowed_file(file.filename):
            ...
            profile_picture_path = os.path.join(current_app.config['UPLOAD_FOLDER'], profile_picture_filename)
...
def handle_invalid_character_data():
    ...
    """
    flash('Invalid character data received. Please check the data source or try again later.', 'danger')
...
def handle_missing_character_data():
    ...
    """
    flash('Character data could not be found. Please check the character name and try again.', 'danger')
...
def check_existing_character(character_name, user_id):
    """
    Checks if a character with the given name already exists for the specified admin.
    """
...
def handle_character_update(character):
    ...
    try:
        ...
        if not updated_data['name']:
            flash('Character name is required.', 'danger')
            return redirect(url_for('user_bp.edit_character', character_id=character.id))
        ...
        return redirect(url_for('user_bp.my_character_list'))  # Redirect to the character list page
